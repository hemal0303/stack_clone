...
def post_question(request, question_id):
    ...
    if request.method == "POST":
        form = QuestionForm(request.POST, instance=post) if post else QuestionForm(request.POST)
        if form.is_valid():
...
def view_question(request, question_id):
    try:
        ...
        if question_id:
            vote_count = None
            voted = False
            data = Post.objects.filter(id=question_id).values("id", "title", "body", "votes").first()
            vote_count = (
...
def vote_question(request, question_id):
    try:
        ...
        if request.method == "POST":
            ...
            voted = False
            if question_id:
                post_owner = Post.objects.filter(id=question_id).values("author_id").first()
                if post_owner["author_id"] == current_user_id:
                    return JsonResponse({"code": 0, "msg": "You cant vote your own question"})
                existing_vote = Vote.objects.filter(question=question_id, user_id=current_user_id).values("vote_type").first()
                if not existing_vote:
                    Vote.objects.create(question_id=question_id, user_id=current_user_id, vote_type=new_vote_type)
                    voted = True
                if existing_vote and existing_vote["vote_type"] == "up" and new_vote_type == "up":
                    Vote.objects.filter(question=question_id, user_id=current_user_id).delete()
                    voted = False
                elif existing_vote and existing_vote["vote_type"] == "down" and new_vote_type == "up":
                    Vote.objects.filter(question=question_id, user_id=current_user_id).update(vote_type=new_vote_type)
                    voted = True
                elif existing_vote and existing_vote["vote_type"] == "up" and new_vote_type == "down":
                    Vote.objects.filter(question=question_id, user_id=current_user_id).update(vote_type=new_vote_type)
                    voted = True
                elif existing_vote and existing_vote["vote_type"] == "down" and new_vote_type == "down":
                    Vote.objects.filter(question=question_id, user_id=current_user_id).delete()
                    voted = False
                else:
                    pass
                ...
                if len(vote_count) >= 1:
                    ...
                    vote_count = str(0)
                return JsonResponse({"code": 1, "vote_count": str(vote_count), "voted": voted})
