...
def profile(request):
    try:
        ...
        if user_id:
            user_data = Profile.objects.filter(user_id=user_id).values(
                "user_id",
                "user__first_name",
                "user__last_name",
                "user__email",
...
def save_profile(request,user_id):
    try:
        ...
        User.objects.filter(id=user_id).update(first_name=firstname,last_name=lastname,email=email,username=username)
        Profile.objects.filter(user_id=user_id).update(avatar=img_name,github_link=github_link,twitter_link=twitter_link,website_link=website_link)
